...
public class enum_value_analyzer extends declaration_analyzer implements variable_declaration {
  ...
  public type value_type() {
    return declared_in_type().get_flavored(flavors.deeply_immutable_flavor);
  }

  @Override
  public type reference_type() {
    return library().get_reference(flavors.deeply_immutable_flavor, value_type());
  }
  ...
  protected @Nullable error_signal do_multi_pass_analysis(analysis_pass pass) {
    ...
    if (pass == analysis_pass.BODY_CHECK && parameters != null) {
      origin pos = this;
      analyzable allocate = new analyzable_action(
          new allocate_action(declared_in_type(), pos), pos);
